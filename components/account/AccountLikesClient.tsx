  ...
  return (
    <>
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="mb-1 text-2xl font-bold tracking-tight">{t('library')}</h1>
          <p className="text-sm text-muted-foreground">{t('library_desc')}</p>
        </div>
        ...
        <div className="relative w-full max-w-sm">
          ...
          <input
            ...
            placeholder={type === 'prompts' ? t('search_in_prompts') : t('search_in_pets')}
      ...
      <div className="mb-8 flex items-center justify-between border-b border-border pb-2">
        ...
        <p className="text-[10px] font-mono text-muted-foreground uppercase tracking-wider">
          {query ? t('matched', { count: filteredCount }) : t('total_count', { count: totalCount })}
        </p>
      </div>
      ...
      {filteredCount === 0 ? (
        query ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="mb-1 text-sm font-medium">{t('no_results', { query })}</p>
            <p className="text-xs text-muted-foreground">{t('try_different')}</p>
          </div>
        ) : (
          <div className="relative flex flex-col items-center justify-center overflow-hidden rounded-xl border border-dashed border-border px-6 py-14 text-center">
            ...
            <p className="mb-1 text-sm font-semibold">
              {type === 'prompts' ? t('no_liked_prompts') : t('no_liked_pets')}
            </p>
            <p className="mb-8 max-w-xs text-xs leading-5 text-muted-foreground">
              {type === 'prompts' ? t('heart_prompt_hint') : t('heart_pet_hint')}
            </p>
